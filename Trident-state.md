...
This is a pretty easy type of spout to understand, the stream is divided into fixed batches that never change. trident-kafka has [an implementation of a transactional spout](https://github.com/nathanmarz/trident-kafka/blob/master/src/jvm/trident/kafka/TransactionalTridentKafkaSpout.java) for Kafka.
...
[OpaqueTridentKafkaSpout](https://github.com/nathanmarz/trident-kafka/blob/master/src/jvm/trident/kafka/OpaqueTridentKafkaSpout.java) is a spout that has this property and is fault-tolerant to losing Kafka nodes. Whenever it's time for OpaqueTridentKafkaSpout to emit a batch, it emits tuples starting from where the last batch finished emitting. This ensures that no tuple is ever skipped or successfully processed by multiple batches.
...
[MemoryMapState](https://github.com/nathanmarz/storm/blob/master/src/jvm/storm/trident/testing/MemoryMapState.java) and [MemcachedState](https://github.com/nathanmarz/trident-memcached/blob/master/src/jvm/trident/memcached/MemcachedState.java) each implement both of these interfaces.
...
OpaqueMap's will call multiPut with [OpaqueValue](https://github.com/nathanmarz/storm/blob/master/src/jvm/storm/trident/state/OpaqueValue.java)'s for the vals, TransactionalMap's will give [TransactionalValue](https://github.com/nathanmarz/storm/blob/master/src/jvm/storm/trident/state/TransactionalValue.java)'s for the vals, and NonTransactionalMaps will just pass the objects from the topology through.

Trident also provides the [CachedMap](https://github.com/nathanmarz/storm/blob/master/src/jvm/storm/trident/state/map/CachedMap.java) class to do automatic LRU caching of map key/vals.

Finally, Trident provides the [SnapshottableMap](https://github.com/nathanmarz/storm/blob/master/src/jvm/storm/trident/state/map/SnapshottableMap.java) class that turns a MapState into a Snapshottable object, by storing global aggregations into a fixed key.
